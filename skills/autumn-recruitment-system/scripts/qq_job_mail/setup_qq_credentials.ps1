...
if ([string]::IsNullOrWhiteSpace($DataDir)) {
    if (-not [string]::IsNullOrWhiteSpace($env:AUTUMN_RECRUITMENT_MAIL_DATA_DIR)) {
        ...
        if ([string]::IsNullOrWhiteSpace($userProfile)) {
            throw 'Cannot resolve the current Windows user profile.'
        }
        $DataDir = Join-Path $userProfile 'AppData\Local\Codex\autumn-recruitment-system\qq_job_mail'
    }
}
...
$form.Text = 'Connect QQ Mail (read-only)'
...
$intro.Text = "Enter the QQ email address and IMAP/SMTP authorization code.`r`nDo not enter the QQ login password. The code is encrypted with Windows DPAPI."
...
$emailLabel.Text = 'QQ email address'
...
$codeLabel.Text = 'Authorization code'
...
$saveButton.Add_Click({
    ...
    if ($address -notmatch '^[^@\s]+@(qq\.com|foxmail\.com)$') {
        $statusLabel.Text = 'Enter a valid @qq.com or @foxmail.com address.'
        return
    }
    if ([string]::IsNullOrWhiteSpace($authCode)) {
        $statusLabel.Text = 'Enter the IMAP/SMTP authorization code generated by QQ Mail.'
        return
    }
